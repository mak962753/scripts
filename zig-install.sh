...
echo "Getting the latest Zig master build URL from ziglang.org/download/index.json..."

QUERY=".master.\"$ZIG_ARCH\".tarball"
echo "Fetching latest Zig master build information($QUERY)..."
DOWNLOAD_URL=$(curl -s "https://ziglang.org/download/index.json" | jq -r "$QUERY")
...
TEMP_FILE=$(mktemp)
curl -L -o "$TEMP_FILE" "$DOWNLOAD_URL" || { echo "Download failed!"; rm -rf "$TEMP_FILE"; exit 1; }
...
echo "Checking PATH to include Zig BIN..."
